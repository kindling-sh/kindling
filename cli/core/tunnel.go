...
// ── Stable tunnel support ───────────────────────────────────────
// When a user configures a stable domain (e.g. via ngrok's free static
// domain), kindling stores it so the public URL stays the same across
// sessions. OAuth providers only need to be configured once.

// StableTunnelConfig holds persistent configuration for a stable tunnel.
type StableTunnelConfig struct {
	Domain string // e.g. "myapp-dev.ngrok-free.app"
}

// SaveStableTunnelConfig persists the user's chosen domain to
// .kindling/tunnel-config.yaml so subsequent runs reuse it.
func SaveStableTunnelConfig(config *StableTunnelConfig) error {
	cwd, err := os.Getwd()
	if err != nil {
		return err
	}
	...
	content := fmt.Sprintf("# Stable tunnel domain — generated by kindling expose --domain\ndomain: %s\n", config.Domain)

	path := filepath.Join(kindlingDir, "tunnel-config.yaml")
...
// ReadStableTunnelConfig loads stable tunnel configuration from
// .kindling/tunnel-config.yaml. Returns nil (no error) if the file
// doesn't exist — the user simply hasn't configured a stable domain.
func ReadStableTunnelConfig() (*StableTunnelConfig, error) {
	cwd, err := os.Getwd()
	if err != nil {
		return nil, err
	}
	data, err := os.ReadFile(filepath.Join(cwd, ".kindling", "tunnel-config.yaml"))
	if err != nil {
		return nil, err
	}

	config := &StableTunnelConfig{}
	for _, line := range strings.Split(string(data), "\n") {
		line = strings.TrimSpace(line)
		...
		if strings.HasPrefix(line, "domain:") {
			config.Domain = strings.TrimSpace(strings.TrimPrefix(line, "domain:"))
		}
	}
	return config, nil
}

// StartStableNgrokTunnel starts an ngrok tunnel with a fixed static domain.
// The domain must be claimed in the ngrok dashboard (one free per account).
func StartStableNgrokTunnel(port int, domain string) (*TunnelResult, error) {
	tunnelCmd := exec.Command("ngrok", "http",
		fmt.Sprintf("%d", port),
		"--domain", domain,
		"--log", "stdout",
	...
	// With a static domain the URL is known upfront, but we still wait
	// for ngrok to confirm the tunnel is online via its local API.
	...
	if !confirmed {
		if tunnelCmd.Process == nil {
			return nil, fmt.Errorf("ngrok process died before the tunnel came online")
		}
	}
	...
	result := &TunnelResult{
		PublicURL: publicURL,
		PID:       tunnelCmd.Process.Pid,
		DNSOK:     true, // static domains have stable DNS
	}
