...
// ── Stable callback domain support ──────────────────────────────
// When a user configures a stable callback domain, kindling creates
// an Ingress in the cluster that routes paths to the correct services.
// An external Cloudflare tunnel (managed outside kindling) routes the
// domain to localhost:80 where Traefik picks up the callback Ingress.
// Configure your OAuth callback URL once — it survives across sessions.

// CallbackRoute maps a path to a resolved service name and port.
type CallbackRoute struct {
	Service string
	Port    int32
}

// StableTunnelConfig holds persistent configuration for callback routing.
type StableTunnelConfig struct {
	Domain string            // e.g. "dev.myapp.com"
	Routes map[string]string // path → short service name (e.g. "/auth" → "gateway")
}

// SaveStableTunnelConfig persists the callback domain and routes to
// .kindling/tunnel-config.yaml so subsequent runs reuse them.
func SaveStableTunnelConfig(config *StableTunnelConfig) error {
	...
	content := fmt.Sprintf("# Callback domain — generated by kindling expose --domain\ndomain: %s\n", config.Domain)
	if len(config.Routes) > 0 {
		content += "routes:\n"
		for path, svc := range config.Routes {
...
// ReadStableTunnelConfig loads callback domain configuration from
// .kindling/tunnel-config.yaml. Returns nil (no error) if the file
// doesn't exist — the user simply hasn't configured a callback domain.
func ReadStableTunnelConfig() (*StableTunnelConfig, error) {
	...
	for _, line := range strings.Split(string(data), "\n") {
		...
		if inRoutes && strings.HasPrefix(line, "  ") {
			// Parse "  /path: service"
		...
		if strings.HasPrefix(trimmed, "domain:") {
			config.Domain = strings.TrimSpace(strings.TrimPrefix(trimmed, "domain:"))
		}
	}
	return config, nil
}
